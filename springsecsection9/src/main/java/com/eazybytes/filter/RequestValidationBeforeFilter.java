    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(header != null){
            ...
            if(StringUtils.startsWithIgnoreCase(header,AUTHENTICATION_SCHEMA_BASIC)){
                byte[] base64Token = header.substring(6).getBytes(credentialsCharset); // 6 is the length of 'Basic' and space
                byte[] decoded;
                try{
                    ...
                    if(email.toLowerCase().contains("test")){
                        res.setStatus(HttpServletResponse.SC_BAD_REQUEST); // SC_BAD_REQUEST = 400
                        return;
                    }
